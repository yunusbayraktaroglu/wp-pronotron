...
/**
 * @original https://raw.githubusercontent.com/WordPress/WordPress/master/wp-admin/includes/image-edit.php
 * 
 * Called by customized imageEditor js library with ajax, open & save function
 * 
 */
...
function wp_image_editor( $post_id, $msg = false ) {
	...
	//DevelopmentUtils::debug_log( $backup_sizes );
	...
	<div class="imgedit-wrap wp-clearfix">
		...
		<div id="imgedit-panel-<?php echo $post_id; ?>">
			...
			<div class="imgedit-settings">
				...
				<div class="imgedit-group" style="display: none;">
					...
					<div class="imgedit-group-top">
						...
						<button type="button" class="dashicons dashicons-editor-help imgedit-help-toggle" onclick="imageEdit.toggleHelp(this);" aria-expanded="false">
			...
			<?php /** END OF RIGHT PANEL */ ?>



			<?php /** RIGHT THUMBNAIL SETTINGS PANEL <------------------- */

			if ( $thumb && $sub_sizes ) {

				$thumb_img = wp_constrain_dimensions( $thumb['width'], $thumb['height'], 160, 120 );
				$pronotron_images = get_option( "wp_pronotron_registered_images" );

				//DevelopmentUtils::debug_log( $pronotron_images );

				$figures = '';
				foreach( $pronotron_images as $image ){
					$img 	= $image[ 'registered' ][ 1 ];
					$url 	= image_get_intermediate_size( $post_id, $img );
					$label	= $image[ 'label' ];

					$title = "<figcaption class='imgedit-thumbnail-preview-caption'>{$label}</figcaption>";
					$img_tag = "<img src='{$url['url']}' width='100%' height='auto' draggable='false' />";
					$figures .= '<figure class="imgedit-thumbnail-preview">' . $title . $img_tag . '</figure>';
				}
				...
				<div class="imgedit-group imgedit-applyto">
					<div id="imgedit-save-target-<?php echo $post_id; ?>" class="imgedit-save-target">
						<fieldset>
							<legend><?php _e( 'Crop to:' ); ?></legend>
							<?php wp_pronotron_print_ratio_buttons( $post_id, $nonce ); ?>
			...
			</div><?php /** END imgedit-settings */ ?>




		</div><?php /** END imgedit-panel */ ?>
...
function wp_save_image( $post_id ) {
	...
	// 	[target] => [\"portrait_3_5_full\",\"portrait_3_5_medium\"]
	// 	[context] => 
	...
	/**
	 * wp_get_image_editor fetchs image editor classes(imagick, gd) and return
	 * new imageeditor()->load() or WP_Error
	 */
	...
	//DevelopmentUtils::debug_log( $img );
	 ...
	 * $backup_sizes -> returns null if any edit not yet done
	 */
	$meta         = wp_get_attachment_metadata( $post_id );
	...
	// new path: [31-Mar-2023 07:23:10 UTC] /var/www/html/wp-content/uploads/test-e1680247390323.jpg
	// DevelopmentUtils::debug_log( $new_path );


	/**
	 * Creates image file exact size of crop area with suffix
	...
	/**
	 * Define "$sizes" to use in following code
	 * Get form request from radio buttons (thumbnail, [landscape_3_5_full, portrait...])
	 */
	if ( 'thumbnail' === $target ){
	...
	/*
	 * We need to remove any existing "cropped" image files because
	 * a new crop or rotate could generate different sizes (and hence, filenames),
	 * keeping the new cropped images from overwriting the existing image files.
	...
	/**
	 * If sizes defined in request ( [landscape, portrait] )
	 */
	if ( isset( $sizes ) ){
