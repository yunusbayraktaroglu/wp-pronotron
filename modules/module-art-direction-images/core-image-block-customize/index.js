			...
			{ isFluid ? (
				<figure {...blockProps}>
					<BlockContextProvider value={{ allowResize: false, imageCrop: false }}>
						<ArtImage sizes={ imageData.media_details.sizes } />
...
function ArtImage({ sizes }){
	
	if ( !sizes ) return null;

	return (
		<div className='wp-pronotron-fluid-image'>
			<picture className='art-direction-image'>
				<source srcSet={ sizes.landscape_5_3_medium.source_url } sizes="1x" media="(orientation: landscape)" />
				<source srcSet={ sizes.portrait_3_5_medium.source_url } sizes="1x" media="(orientation: portrait)" />
				<img src={ sizes.full.source_url } />
			</picture>
		</div>
	)
}